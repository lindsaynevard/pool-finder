...
async function getClosedDates() {
  try {
    ...
    console.warn(`  Lions Pool: could not fetch closure dates: ${err.message}`);
    return new Set();
  }
}

export async function scrapeLions(daysAhead = 14) {
  const results = {};
  ...
  if (closedDates.size > 0) {
    console.log(`  Lions Pool closed dates from Oakland website: ${[...closedDates].join(', ')}`);
  }
  ...
  for (let i = 0; i < daysAhead; i++) {
    ...
    if (dow === 0 || dow === 6) {
      ...
      sessions = [{ start: '11:00 AM', end: '2:30 PM', type: 'lap', notes: null }];
    } else if (dow >= 1 && dow <= 4) {
      // Mon–Thu: three sessions
      sessions = [
        { start: '6:30 AM',  end: '9:00 AM',  type: 'lap', notes: null },
        { start: '12:30 PM', end: '2:30 PM',  type: 'lap', notes: null },
        { start: '7:30 PM',  end: '9:00 PM',  type: 'lap', notes: null },
      ];
    } else {
      // Fri: midday only
      sessions = [{ start: '12:30 PM', end: '2:30 PM', type: 'lap', notes: null }];
    }
